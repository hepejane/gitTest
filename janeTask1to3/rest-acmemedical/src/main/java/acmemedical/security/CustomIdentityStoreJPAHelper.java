...
public class CustomIdentityStoreJPAHelper {
    ...
    public SecurityUser findUserByName(String username) {
         ...
         */
        try {
            // Use NamedQuery to fetch the user by username
            TypedQuery<SecurityUser> query = em.createNamedQuery("SecurityUser.userByName", SecurityUser.class);
            query.setParameter("param1", username);  // Set the parameter for username
            user = query.getSingleResult();  // Get the single result (will throw NoResultException if not found)
